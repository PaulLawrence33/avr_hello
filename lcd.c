...
static const uint16_t fontData9x16[FONT_CHARS][FONT_WIDTH] PROGMEM = {
...
static int i2c_status(int flags)
{
...
static void i2c_send(uint8_t byte)
{
	TWDR = byte;
...
static void lcd_command1(uint8_t command, uint8_t data)
{
	i2c_start();
	i2c_send(0x00);
	i2c_send(command);
...
static void lcd_string(uint8_t x, uint8_t y, const char *s)
{
